...
Dans les chapitres 4, 5 et 6, je décrirai une autre méthode pour écrire les mots-clés ains que les notations par étoiles et couleurs. Mais cette fenêtre constitue le seul moyen disponible dans XnViewMP pour modifier toutes les autres métadonnées (Légende, Titre, Copyright, Ville, Pays, etc).
...
Vous pouvez ainsi en 10 secondes entrer la ville, le pays et la région sur des centaines d'images. Répétez l'opération si quelques dizaines d'images peuvent recevoir la même "légende", etc.

Tout ceci reste valable, mais en décembre 2020 la version V0.98 est apparue en apportant de nouvelles fonctionnalités. Si vous n'utilisez que les quelques métadonnées pratiques pour le grand public, vous pouvez passer directement à la suite au chapitre 3A.
...
Vous ouvrez avec ce nouveau menu une fenêtre d'édition dans laquelle une part importante des champs nouvellement ajoutés est consacrée à ces notions d'auteur et de droits. Comme pour "l'ancienne" méthode décrite dans les chapitres 3A, 3B et 3C, le dernier onglet `Options` est à paramétrer.
...
La première option: `Conserver et ajouter la nouvelle valeur` (Mots clés et catégories supplémentaires). Elle doit rester cochée pour préserver des mots clés existants de tout effacement accidentel et permettre d'ajouter des mots-clés à ceux existants. Si on la décoche, j'étudierai ça plus tard. Oubliez ici la présence du mot "catégories", métadonnée obsolète.
...
N'oubliez surtout pas de remettre la deuxième option une fois la manipulation faite. Si vous voulez effacer toutes les métadonnées, y compris les EXIF, passez dans le menu `Outils, Métadonnées, Nettoyer`.
...
Entrez la ou les métadonnées souhaitées (Auteur, copyright, etc) et cliquez le bouton `Ecrire les fichiers`. C'est une méthode extrêmement rapide pour entrer une métadonnée dans un lot de photos.
...
- troisième onglet `Catégories`. Ce champ n'est plus utilisé, on l'oublie. Et ça n'a aucun rapport avec les "catégories" de XnViewMP qu'on va utiliser;
...
- sixième onglet `Date et heure`. Il s'agit ici de renseigner des champs IPTC ou XMP avec une date. Ces champs me paraissent peu utilisés, je suppose que l'intérêt de les utiliser concerne les dates historiques qui sont une donnée à part entière. En fait, les autres dates "sytème" ne devraient pas être antérieures à 1970 sous peine de planter certains logiciels.
...
Les mots clés doivent être sous forme de liste (un par ligne) pour respecter les normes IPTC et XMP. Parfois, d'anciens mots clés récupérés d'autres logiciels sont visibles en une seule ligne avec des virgules de séparation: ça fonctionne plus ou moins, il faudra de préférence y remédier. La croix rouge permet d'effacer un mot clé sélectionné, si vous voulez en ôter un dans cette zone. Ne tentez pas trop de modifications compliquées sur des lots de photos, car le résultat n'est pas toujours celui escompté. Faites plutôt des ajouts ou des suppressions, sur des lots homogènes de photos.
...
Je considère qu'on va se contenter avec cette fenêtre de renseigner les champs `Légende, Titre et Ville` pour "une première passe" dans les métadonnées. C'est déjà pas mal. Ne soyez pas trop ambitieux sur le nombre de métadonnées à renseigner, car vous risquez de vous décourager au bout de quelques dizaines de photos.
...
Exemple: vous sélectionnez 10 images, vous cliquez `Effacer les champs`, vous entrez une ville puis `Ecrire les fichiers`. Tous les autres champs étant vides, ils ne seront pas modifiés. Seul le champ Ville n'est pas vide, "Lyon" sera écrit dans les 10 photos.
...
- si le format d'image ne supporte pas la norme IPTC, le mode `IPTC + mise à jour XMP` écrit les XMP mais ne lit rien.

### Comment sont visualisés les mots clés EXIF IPTC XMP dans la zone Informations

1) EXIF: il s'agit d'une seule chaine de caractères (un seul mot donc), même s'il contient des caractères de séparation. Il se nomme EXIF:XpKeywords et a été utilisé par Microsoft; il est obsolète, ce champ n'est plus utilisé par les logiciels récents.
...
- Forme EXIF: et juste pour info, la forme EXIF ancienne, en un seul mot, avec ou sans virgules, non gérée par XnViewMP: EXIF:XPKeywords animal,oiseau,moineau. Si vous voulez récupérer et "rénover" ce genre de mot-clés utilisés dans le passé, on peut utiliser un script pour ExifTool. Allez voir l'exemple 4 du chapitre 11N.
...
- choisissez `Charger un modèle` puis (ModèleTemp) et cliquez sur le bouton `Ecrire`. Notez que d'avoir décoché la deuxième case de l'onglet `Options` permet d'effacer les métadonnées qui étaient dans l'image cible.
