...
Depuis la version V0.98, il existe une autre fenêtre aux fonctions symétriques nommée `Éditer XMP`. Pour le moment, ignorons-la car elle concerne des métadonnées XMP spécifiques qu'on n'utilisera pas.
...
Il faut sélectionner au moins deux images pour pouvoir agir sur ces paramètres de sélection multiple. Ensuite le paramètre `Mode`, choisissez `IPTC-IIM, création ou mise à jour XMP`. Ça permet de garder les mêmes informations en IPTC et XMP.
